"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : new P(function (resolve) { resolve(result.value); }).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
var _this = this;
Object.defineProperty(exports, "__esModule", { value: true });
var user_1 = __importDefault(require("../models/user"));
var bcrypt_1 = __importDefault(require("bcrypt"));
var jsonwebtoken_1 = __importDefault(require("jsonwebtoken"));
var config_1 = require("../config");
// import { mailTransport } from './mails';
exports.login = function (req, res) { return __awaiter(_this, void 0, void 0, function () {
    var user, matches, e_1;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                _a.trys.push([0, 3, , 4]);
                return [4 /*yield*/, user_1.default.findOne({
                        email: req.body.email
                    })
                        .select('passwordHash')
                        .exec()];
            case 1:
                user = _a.sent();
                if (!user) {
                    return [2 /*return*/, res
                            .status(401)
                            .json({
                            message: 'Email or Password wrong, please try again.'
                        })];
                }
                return [4 /*yield*/, bcrypt_1.default.compare(req.body.password, user.passwordHash)];
            case 2:
                matches = _a.sent();
                if (!matches) {
                    res.status(401).json({
                        message: 'Email or Password wrong, please try again.'
                    });
                }
                else {
                    res.status(200).json(createToken(user));
                }
                return [3 /*break*/, 4];
            case 3:
                e_1 = _a.sent();
                console.log(e_1);
                return [2 /*return*/, res.status(500).json(e_1)];
            case 4: return [2 /*return*/];
        }
    });
}); };
exports.signup = function (req, res) { return __awaiter(_this, void 0, void 0, function () {
    var duplicateUser, message, hash, newUser, user, e_2;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                _a.trys.push([0, 4, , 5]);
                if (!req.body.password) {
                    return [2 /*return*/, res.status(400).json({ message: 'Password is required!' })];
                }
                if (req.body.password != req.body.confirmPassword) {
                    return [2 /*return*/, res.status(400).json({ message: "Passwords don't match!" })];
                }
                return [4 /*yield*/, user_1.default.findOne({
                        $or: [{ email: req.body.email }, { name: req.body.name }]
                    }).exec()];
            case 1:
                duplicateUser = _a.sent();
                if (duplicateUser) {
                    message = void 0;
                    if (duplicateUser.name == req.body.name &&
                        duplicateUser.email == req.body.email) {
                        message = 'Name and E-Mail are already taken';
                    }
                    else if (duplicateUser.name == req.body.name) {
                        message = 'Name is already taken, pick another one';
                    }
                    else {
                        message =
                            'E-Mail is already taken, try logging in or reset your Password';
                    }
                    return [2 /*return*/, res.status(409).json({ message: message })];
                }
                return [4 /*yield*/, bcrypt_1.default.hash(req.body.password, 10)];
            case 2:
                hash = _a.sent();
                newUser = new user_1.default({
                    name: req.body.name,
                    email: req.body.email,
                    passwordHash: hash,
                    createdOn: new Date()
                });
                return [4 /*yield*/, newUser.save()];
            case 3:
                user = _a.sent();
                // await mailTransport.sendMail({
                //     from: 'Flo',
                //     to: req.body.email,
                //     subject: 'Sign up successful!',
                //     text: `Welcome ${
                //         req.body.name
                //     }, you successfuly signed up on this Website`,
                //     sender: 'Meeee!'
                // });
                res.status(201).json(createToken(user));
                return [3 /*break*/, 5];
            case 4:
                e_2 = _a.sent();
                return [2 /*return*/, res.status(500).json(e_2)];
            case 5: return [2 /*return*/];
        }
    });
}); };
exports.get_me = function (req, res) { return __awaiter(_this, void 0, void 0, function () {
    var user;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                if (!req.userData) return [3 /*break*/, 2];
                return [4 /*yield*/, user_1.default.findById(req.userData._id).exec()];
            case 1:
                user = _a.sent();
                res.status(200).json(user);
                return [3 /*break*/, 3];
            case 2: return [2 /*return*/, res.status(403).json({ message: 'No Permission for this action' })];
            case 3: return [2 /*return*/];
        }
    });
}); };
//TODO: Make Admin only
exports.get_all = function (req, res) { return __awaiter(_this, void 0, void 0, function () {
    var users;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                if (!req.userData) return [3 /*break*/, 2];
                return [4 /*yield*/, user_1.default.find().exec()];
            case 1:
                users = _a.sent();
                return [2 /*return*/, res.status(200).json(users)];
            case 2: return [2 /*return*/, res.status(403).json({ message: 'No Permission for this action' })];
        }
    });
}); };
exports.verify_name = function (req, res) { return __awaiter(_this, void 0, void 0, function () {
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0: return [4 /*yield*/, user_1.default.findOne({ name: req.params.name })];
            case 1:
                if (_a.sent()) {
                    res.status(400).send();
                }
                else {
                    res.status(200).send();
                }
                return [2 /*return*/];
        }
    });
}); };
function createToken(user) {
    return {
        token: jsonwebtoken_1.default.sign({ email: user.email, _id: user._id, isAdmin: user.isAdmin }, config_1.config.JWT_SECRET, { expiresIn: '1h' }),
        _id: user._id,
        name: user.name,
        expires: new Date().addHours(1)
    };
}
